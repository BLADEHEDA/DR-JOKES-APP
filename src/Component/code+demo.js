...
// jokes card to display jokes that takes 2 arguments as props (joke and new comment)
const JokeCard = ({ joke, onNewComment }) => {
  ...
  const onSubmit = () => {
    // alert(text + joke.id);
    // do a post rewuest to the comment API to post the Jokes (DONE)
    setLoading(true);
  ...
  return (
    <div className='border m-4 p-2 bg-light'>
      <p>{joke.punchline}</p>
      <p>{joke.setup}</p>
      {joke.comments.map(comment => <CommentsCard comment={comment} />)}
      <div className='border border-dark rounded'>
        <div>
          <input value={text} className='form-input' onChange={e => setText(e.target.value)} />
        </div>
        <button className='btn btn-primary' onClick={onSubmit}>{loading ? 'Commenting ...' : 'Comment'}</button>
      </div>
    </div>
  )
}
// Add comments to the particular jokes 
function Jokes() {
